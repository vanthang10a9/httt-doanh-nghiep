...
if (isset($_POST['order-action'])) {
    ...
    if ($_POST['order-action'] == "add") {
        ...
        while ($row_detail = mysqli_fetch_assoc($run_detail)) {
            $SOLUONG = $row_detail['SOLUONG'];
            $MASP = $row_detail['MASP'];
            $sql_sl = DataProvider::executeQuery("SELECT SOLUONGSP FROM sanpham WHERE MASP = '$MASP'");
            $S = mysqli_fetch_assoc($sql_sl);

            $SOLUONGSP = $S['SOLUONGSP'];
            if ($SOLUONGSP - $SOLUONG >= 0) {
                if ($SOLUONGSP - $SOLUONG < 10) {
                    $sql_product = "UPDATE sanpham SET DUYET = 1, SOLUONGSP = SOLUONGSP - '$SOLUONG' WHERE MASP='$MASP'";
                } else {
                    $sql_product = "UPDATE sanpham SET SOLUONGSP = SOLUONGSP - '$SOLUONG' WHERE MASP='$MASP'";
                }
                DataProvider::executeQuery($sql);
                $run = DataProvider::executeQuery($sql_product);
            }
            else {
                $run = 0;
            }
